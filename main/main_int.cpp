    ...
    while (opcion != 3){  // Mientras no se elija la opción para salir...
        ...
        switch (opcion){
            case 1:{  // Si se ha elegido el modo demostración...
                ...
                ABB_t<int> ABB;  // Se crea el ABB vacío
                cout << "HECHO!" << endl;
                cout << endl;
                ...
                while (opcion_demo != 3){  // Mientras no se elija la opción para salir...
                    ...
                    switch (opcion_demo){
                        case 1:{  // Si se quiere insertar una clave...
                            int aux;  // Variable que contendrá el valor a introducir
                            cout << "Introduzca la clave a insertar: ";
                            cin >> aux;
                            ABB.insertar(aux);  // Se inserta la clave en el ABB
                            cout << endl;
                        ...
                        case 2:{  // Si se quiere eliminar una clave...
                            int aux;  // Variable que contendrá el valor de la clave a eliminar
                            cout << "Introduzca la clave a eliminar: ";
                            cin >> aux;
                            ABB.eliminar(aux);  // Se elimina la clave del ABB
                            cout << endl;
            ...
            case 2:{  // Si se ha elegido el modo estadístico...
                ...
                int aux;  // Entero auxiliar para guardar las claves generadas aleatoriamente
                bool iguales;  // Variable auxiliar para evitar que se guarden claves repetidas en el banco de pruebas
                vector<int> pos;  // Vector auxiliar para guardar las posiciones que se evaluarán del banco de pruebas
                vector<int> banco_pruebas;  // Banco de pruebas que contendrá las claves a insertar y buscar
                ABB_t<int> ABB;  // Se crea el ABB vacío
                ...
                // Se rellena el banco de pruebas con claves aleatorias
                for (int i=0; i<banco_pruebas.size(); i++){
                    iguales = true;
                    while (iguales){
                        // Se genera un entero aleatorio
                        aux = rand() % 100000;

                        iguales = false;
                        int j = 0;
                        // Se comprueba que la clave generada no esté repetida
                        while ((j<i) && (!iguales)){  // Mientras no se haya recorrido todas las claves introducidas y no se haya encontrado ninguna coincidencia...
                            // Si se encuentra una clave igual en el banco de pruebas, se sale del bucle y se genera otra de forma aleatoria
                        ...
                        // Si la clave no está repetida, se añade al banco de pruebas
                        if (!iguales){
                            banco_pruebas[i] = aux;
                        }
                    }
                ...
                // Se insertan en el árbol las N primeras claves del banco de pruebas
                for (int i=0; i<N; i++){
